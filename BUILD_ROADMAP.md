...
## Taxonomy
- **Todo** — a task. Has: title, estimated_minutes, bucket, optional area_id, optional notes.
...
- **Area** — life/work context: "CompanyX", "Uni", "Health", "Music", "Me Time". Flat list v2.0; optional grouping v2.1+ if cluttered.
- **Area ↔ Calendar link** — an Area can link to 0+ calendars (iCloud/Google). Links drive automatic time tracking.
...
- **Multi-Area calendars via hashtags:** If one calendar hosts multiple Areas (e.g. one "Personal" calendar for yoga, music, dinner), event titles with `#health` / `#music` route to the right Area. Same regex parser used by Notion/Reminders integration — free code reuse.
...
7. **Areas** — create/edit, optional time budgets. Calendar linking lives here too.
